...
func NewMetadataWaitStrategy(timeout time.Duration) *MetadataWaitStrategy {
	return &MetadataWaitStrategy{
...
func (mw *MetadataWaitStrategy) getKafkaMetadata(ctx context.Context, brokers []string) bool {
	...
	if err != nil {
		mw.LogDebug(ctx, "[TESTCONTAINER] Kafka client creation error:"+err.Error())
		return false
	}
	defer prodClient.Close() //nolint:errcheck
	b := prodClient.Brokers()
	if len(b) < 1 {
		mw.LogDebug(ctx, "[TESTCONTAINER] Got empty broker list")
		return false
	}
	return true
}
